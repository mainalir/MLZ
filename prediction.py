...
def model_predict(X):
    ...
    cat_encoder = load(
        open(pkg_resources.resource_filename(__name__, "model/cat_encoder.pkl"), "rb")
    )
    model = load(
        open(pkg_resources.resource_filename(__name__, "model/model.pkl"), "rb")
    )
    scaler = load(
        open(pkg_resources.resource_filename(__name__, "model/scaler.pkl"), "rb")
    )
    X = np.asarray(X)
    ...
    df_scale.rename(
        columns={0: "Thickness", 1: "Width", 2: "length", 3: "Bottom", 4: "Quantity"},
        inplace=True,
    )
...
def unit_price_with_tax(X):
    ...
    input_file = pd.read_csv(
        pkg_resources.resource_filename(__name__, "model/input_file.csv")
    )
...
def total_unit_price_with_freight(X):
    """Takes the dimension of size of plastic bag and quantity to estimate
    the total volume cbm and freight price.
...
def get_exw2(X):
    """This function returns EXW price after multiplying by a factor which is 
    determined based on 'Digital/Plate', Quantity and width."""
